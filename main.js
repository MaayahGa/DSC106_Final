...
arenas.forEach(arena => {
  ...
  div.addEventListener("click", () => {
    ...
    const filtered = allBattles.filter(b =>
      Number(b["winner.startingTrophies"]) >= arena.trophies
    );

    console.log("Filtering by:", arena.name, "Battles:", filtered.length);
...
function renderChart(battles) {
  ...
  const barHeight = 30;
  const totalHeight = cardData.length * (barHeight + 5);
  ...
  let scrollOffset = 0;
  const maxOffset = Math.max(0, totalHeight - (svgHeight - margin.top - margin.bottom));
